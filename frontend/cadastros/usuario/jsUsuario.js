...
const addUsuarios = async (event) => {
  ...
  const usuarios = {
    usu_nome: inputNome.value,
    usu_login: inputLogin.value,
    usu_senha: inputSenha.value,
    usu_ativo: inputAtivo.checked,
  };
...
const createUsuario = (usuarios) => {
  ...
  const tdDataHoraCadastro = createElement(
    "td",
    formatDate(usu_datahoracadastro)
  );
